...
def generate_hint(answer: str) -> str:
    ...
    masked = ""
    for char in answer[1:]:
        if char.isalnum() or ('ぁ' <= char <= 'ん') or ('ァ' <= char <= 'ン') or ('一' <= char <= '龥'):
            masked += "〇"
        elif char in [" ", "　"]:
            masked += "□"
        else:
            masked += char  # 記号（＠、ー、？など）はそのまま表示
    return f"{first}{masked}（全{len(answer)}文字）"
...
if DROPBOX_URL and not st.session_state.questions:
    with st.spinner("ZIPファイルを読み込み中..."):
        ...
        with zipfile.ZipFile(st.session_state.zip_data) as z:
            ...
            selected = []
            used = set()
            while len(selected) < 50 and all_mp3:
                f = random.choice(all_mp3)
                if f not in used:
                    ans = extract_answer(f).strip()
                    selected.append({"file": f, "answer": ans})
                    used.add(f)
...
if st.session_state.questions:
    ...
    with zipfile.ZipFile(st.session_state.zip_data) as z:
        if not st.session_state.played:
            ...
            st.info("音声は1回のみ再生されます")

    st.subheader(f"🎵 第 {idx + 1} 問")
    st.write("💡 ヒント：" + generate_hint(q["answer"]))
    user_input = st.text_input("答えを入力（ひらがな・英小文字）", key=f"input_{idx}")
